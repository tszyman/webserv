...
bool Router::isCgiRequest(const RequestParser& request) const
{
	...
	if (location == NULL || location->getCgiExtension().empty()
		|| location->getCgiExecutable().empty())
		return false;
	return hasExtension(translatePath(request.getPath(), location),
...
bool Router::handleCgi(const RequestParser& request, const std::string& physicalPath, const LocationConfig* location, HttpResponse& response) const
{
	...
	// The event loop owns these descriptors from this point.  Keeping CGI I/O
	// out of the router prevents a slow CGI request from blocking every client.
...
void Router::handlePost(const RequestParser& request, const LocationConfig* location, HttpResponse& response) const
{
	...
	{
		// A route may explicitly allow POST without being an upload or CGI
		// endpoint (the supplied tester uses /post_body this way).
		response.setStatusCode(200);
		response.setHeader("Content-Type", "text/plain");
		response.setBody("POST request accepted\n");
		return;
	}
